...
- (id)initWithTCPPort:(unsigned)po delegate:(id)dl
{
    if( self = [super init] ) {
        ...
        //NSAssert([delegate respondsToSelector:@selector(processURL:connection:)], @"Delegate needs to implement 'processURL:connection:'");
        //NSAssert([delegate respondsToSelector:@selector(stopProcessing)], @"Delegate needs to implement 'stopProcessing'");
...
- (void)closeConnection:(SimpleHTTPConnection *)connection;
{
    ...
    if( stopProcessing ) {
        if([delegate respondsToSelector:@selector(stopProcessing)])
			[delegate performSelector:@selector(stopProcessing)];
        [self setCurrentRequest:nil];
    }
...
- (void)processNextRequestIfNecessary
{
    if( [self currentRequest] == nil && [requests count] > 0 ) {
        ...
        if([delegate respondsToSelector:@selector(processURL:connection:)])
			[delegate performSelector:@selector(processURL:connection:) withObject:[currentRequest objectForKey:@"url"] withObject:[currentRequest objectForKey:@"connection"]];// processURL:[currentRequest objectForKey:@"url"] connection:[currentRequest objectForKey:@"connection"]];
    }
}
...
- (void)replyWithStatusCode:(int)code data:(NSData *)data MIMEType:(NSString *)type {
	NSDictionary *headers = [NSDictionary dictionaryWithObject:type forKey:@"Content-Type"];
	[self replyWithStatusCode:code headers:headers body:data];
}
