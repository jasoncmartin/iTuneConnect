...
- (NSFileHandle *)fileHandle {
	return [[fileHandle retain] autorelease];
}
...
- (void)dataReceivedNotification:(NSNotification *)notification
{
    ...
    if ( [data length] == 0 ) {
        ...
        if([delegate respondsToSelector:@selector(closeConnection:)])
			[delegate performSelector:@selector(closeConnection:) withObject:self];
    } else {
        [fileHandle readInBackgroundAndNotify];
        ...
        if( success ) {
            if( CFHTTPMessageIsHeaderComplete(message) ) {
				...
				if([delegate respondsToSelector:@selector(newRequestWithURL:connection:)])
					[delegate performSelector:@selector(newRequestWithURL:connection:) withObject:(NSURL *)url withObject:self];
                CFRelease(url);
                CFRelease(message);
                message = NULL;
            } else {
